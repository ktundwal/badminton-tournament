...
// Enumerate every tournament the cloud knows about. Powers the home view
// for a fresh device with an empty localStorage — the user can see and
// join rooms created by anyone else in the group without needing a
// share link.
//
// Uses Redis KEYS which is O(N) over the keyspace. At the scale this app
// runs (20 friends, at most a few hundred rooms ever), it's fine; if
// usage ever balloons, swap to maintaining a SET `cc:rooms` with
// SADD/SREM on create/delete.
export async function listAllRoomIds() {
  if (!isConfigured()) return []
  try {
    const res = await fetch(`${URL_BASE}/keys/cc:room:*`, { headers: authHeaders() })
    if (!res.ok) return []
    const data = await res.json()
    const keys = Array.isArray(data?.result) ? data.result : []
    const prefix = 'cc:room:'
    return keys
      .filter(k => typeof k === 'string' && k.startsWith(prefix))
      .map(k => k.slice(prefix.length))
  } catch (e) {
    console.warn('[cloudsync] listAllRoomIds failed', e)
    return []
  }
}
